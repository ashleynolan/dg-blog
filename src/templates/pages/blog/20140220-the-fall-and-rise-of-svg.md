...
SVG has benefited in no small part to the rise of Responsive Design as developers look for solutions that scale better across multiple devices.  SVG’s are lightweight and scale extremely well when resized, making them an extremely attractive alternative to standard images and icons in certain use cases.

For example, you can use [SVG for iconography](http://ianfeather.co.uk/ten-reasons-we-switched-from-an-icon-font-to-svg/) as explained by [Ian Feather](https://twitter.com/ianfeather) from Lonely Planet.  [The Lonely Planet website](http://www.lonelyplanet.com/) uses SVG for all of it’s iconography, as well as for smaller images such as their logo.

SVG's uses aren’t just limited to icons though.  Many sites are using them across a wide variety of use cases due to their flexibility.  They can be used to replace much larger images, such as on the recent [Lynx Mass Debates site](http://www.lynxmassdebates.com/question/happiness) for providing the visual imagery in each page.


## Implementing SVG as an image replacement technique

SVG's can be exported out of Adobe Illustrator by choosing 'Save as' and then selecting SVG from the Save as type extension drop down.  Once you have your SVG, you then have a number of options.

The simplest way to use them is to reference your SVG's as you would standard images.
...
Including SVG's in this way is fine if you are only including a small number of SVG's in your site, but maintaining a larger number in this way can get tedious, especially if the design of the SVG's change slightly and you have to redo your data-uri's and fallback PNG's each time. Luckily, there is a better solution.

Using Grunt, you can implement a task called Grunticon which was created by the talented guys at Filament Group.  Grunticon let's you specify a folder, or folders, in which you keep you SVG files.  It will go through each of these SVG's and create the CSS needed to include them in your site, as data-uri's or standard SVG's, as well as generating fallback PNG's.  It then gives you a tiny piece of JavaScript to add into your site header which detects relevant feature support and serves up the most appropriate CSS file for that browser to load in your SVG or fallback PNG's.

Grunticon is the best solution I have found for maintaining SVG's in a site, and I'd definitely encourage you to check it out if you want to start using SVG's more in your development.

For a more in-depth look at the ways you can implement SVG’s, take a look at [this article on CSS Tricks](http://css-tricks.com/using-svg/) or [David Bushell’s excellent post on the subject](http://dbushell.com/2013/02/04/a-primer-to-front-end-svg-hacking/).


## SVG for animation

Another strength of SVG that is being used ever more frequently is for use in web animation.  Individual parts of an SVG can be animated using CSS, and techniques such as the one talked about by [Jake Archibald](http://jakearchibald.com/2013/animated-line-drawing-svg/) have been discovered opening up ever more possibilities.

SVG also has some very solid libraries available for drawing and animation, such as Raphael and Snap.SVG.
...
One of the nicest examples I've seen was created by Vox Product, for the Xbox One and Playstation 4 reviews on the Polygon website.  I highly recommend you read the [excellent article they wrote](http://product.voxmedia.com/post/68085482982/polygon-feature-design-svg-animations-for-fun-and) describing how and why they chose to use SVG for the animations on their site, but this snippet from the article describes some of reasons:
...
If anyone has seen any more examples of SVG animation on production websites, please send them over and I’ll add them in to this article.
...
In a lot of my work currently at TMW, I am actively encouraging the use of SVG's to our designers, simply because of how flexible, lightweight and scalable they are in responsive websites.  They are by no means suitable for every situation though and should be used in combination with a robust responsive image solution.
